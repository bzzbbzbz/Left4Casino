...
async def main():
    ...
    chat_restrictions_config = get_config(
        model=ChatRestrictionsConfig, root_key="chat_restrictions"
    )
    ...
    heist_config = get_config(model=HeistConfig, root_key="heist", required=False)
    ...
    dp.message.middleware(
        ThrottlingMiddleware(
            game_config.throttle_time_spin,
            game_config.throttle_time_other,
            game_config.throttle_time_top,
        )
    )
    ...
    async def send_draft_report():
        ...
        await daily_stats_service.generate_and_send_report(
    ...
    async def start_heist():
        ...
        hard_cap_time = datetime.now(timezone) + timedelta(
            minutes=heist_config.max_duration_minutes
        )
        scheduler.add_job(
            end_all_heists,
            "date",
